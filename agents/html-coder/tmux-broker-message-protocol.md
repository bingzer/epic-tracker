...
You are registered as session `html-coder`.
...
> **Important:** Always pass `from: "html-coder"` explicitly in every `send_message` call. The default is `"unknown"`, which causes reply validation to fail with `"only the original recipient may reply"`.
...
> Always pass `from: "html-coder"` on every tool call that accepts it.
