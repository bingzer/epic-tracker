...
You are registered as session `code-reviewer`.
...
> **Important:** Always pass `from: "code-reviewer"` explicitly in every `send_message` call. The default is `"unknown"`, which causes reply validation to fail with `"only the original recipient may reply"`.
...
> Always pass `from: "code-reviewer"` on every tool call that accepts it.
